...
## � **CRITICAL: Integration with General Coding Standards**

**This file defines WHAT to build for BusBuddy. For HOW to build it, see:**
- **`..\.github\copilot-ins#### 💬 **Communication Preferences**
...
#### 🔍 **Existing Code Discovery Protocol**
**MANDATORY: Always check for existing implementations before creating new ones**

**Pre-Development Discovery Steps:**
1. **Search Views Directory**: Use `list_dir` to check `BusBuddy.WPF/Views/` and subdirectories
2. **Search ViewModels**: Use `file_search` with patterns like `*ViewModel.cs` to find existing ViewModels
3. **Search Models**: Check `BusBuddy.Core/Models/` for existing entity definitions
4. **Search Services**: Look in `BusBuddy.Core/Services/` for existing business logic
5. **Grep Search**: Use `grep_search` to find existing implementations of similar functionality

**Discovery Commands:**
```powershell
# Find existing Views and Forms
list_dir "BusBuddy.WPF/Views/"
file_search "**/*View.xaml"
file_search "**/*Form.xaml"

# Find existing ViewModels
file_search "**/*ViewModel.cs"
grep_search "class.*ViewModel" --isRegexp true

# Find existing Models and Services
list_dir "BusBuddy.Core/Models/"
list_dir "BusBuddy.Core/Services/"
grep_search "public class.*Service" --isRegexp true
```

**Before Creating New Components:**
- ✅ **Students functionality**: Check for `StudentsView.xaml`, `StudentForm.xaml`, `StudentsViewModel.cs`
- ✅ **Route management**: Look for `RoutesView.xaml`, `RouteViewModel.cs`, route-related services
- ✅ **Vehicle operations**: Search for existing vehicle forms and ViewModels
- ✅ **Driver management**: Verify driver-related UI components don't already exist
- ✅ **Address validation**: Check for existing `AddressService` or similar validation logic

**Integration Over Recreation:**
- **Extend existing forms** rather than creating duplicates
